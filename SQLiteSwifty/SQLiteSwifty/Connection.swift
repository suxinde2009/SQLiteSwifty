...
extension SQLite {
    ...
    public class Connection {
        ...
        internal let handle: SQLite.DatabaseHandle
        
        /// Sets a busy handler to sleep the specified amount of time when a table is locked.
        public var busyTimeout: Int = 200 {
            didSet {
                SQLite.busyTimeout(handle, milliseconds: busyTimeout)
            }
        }
        ...
        public init(databasePath: String,
                    openFlags: OpenFlags) throws {
            
            self.dbPath = databasePath
            self.openFlags = openFlags
            var dbHandle: SQLite.DatabaseHandle?
            let r = SQLite.open(filename: dbPath, db: &dbHandle, flags: openFlags)
            if r != SQLite.Result.ok {
                throw SQLite.ErrorType.openDataBaseError("Could not open database file at path: \(databasePath), result code: \(r?.rawValue ?? 0)")
            }
            handle = dbHandle!
            _open = true
        }
        ...
        public func createIndex(_ indexName: String,
            ...
            let sql =
                String(
                    format: "CREATE %@ INDEX IF NOT EXISTS %@ ON %@(%@)",
                    columns,
                    indexName,
                    u,
                    tableName
        ...
        public func dropIndex(_ indexName: String,
                              tableName: String) throws -> Int {
        ...
        public func query<T>(_ query: String,
                             parameters: [Any] = []) -> [T] where T: SQLiteCodable {
        ...
        public func findWithQuery<T: SQLiteCodable>(_ sql: String,
                                                    parameters: [Any]) -> T? {
        ...
        /// - Returns: The number of rows modified in the database as a result of this execution.
        /// - Throws: Exceptions
        public func executeScalar<T: SQLiteCodable>(_ query: String,
                                                    parameters: [Any] = []) throws -> T? {
...
extension SQLite.Connection {
    ...
    internal func getMapping<T: SQLiteCodable>(of type: T.Type,
                                               createFlags: CreateFlags = .none) -> SQLite.TableMapping {
        
        let key = String(describing: type)
        var map: SQLite.TableMapping
        lock()
        if let oldMap = _mappings[key] {
            if createFlags != .none && createFlags != oldMap.createFlags {
                map = SQLite.TableMapping(type: type, createFlags: createFlags)
                _mappings[key] = map
            } else {
                map = oldMap
            }
        } else {
            map = SQLite.TableMapping(type: type)
            _mappings[key] = map
        }
        unlock()
        return map
    ...
    internal func migrateTable(_ map: SQLite.TableMapping,
                               existingCols: [SQLite._ColumnInfo]) throws {
        
        var newCols: [SQLite.TableMapping.Column] = []
    ...
    internal func getExistingColumns(tableName: String) -> [SQLite._ColumnInfo] {
        
        let sql = String(format: "pragma table_info(%@)", tableName)
        return query(sql)
    }
    
    internal func createCommand(_ cmdText: String,
                                parameters: [Any]) -> SQLite.Command {
        
        let cmd = SQLite.Command(connection: self)
    ...
    internal func getInsertCommand(map: SQLite.TableMapping,
                                   extra: String) -> SQLite.PreparedSqliteInsertCommand {
        ...
        return SQLite.PreparedSqliteInsertCommand(connection: self, commandText: sql)
    }
